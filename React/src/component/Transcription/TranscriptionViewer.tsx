...
const TranscriptionViewer: React.FC<TranscriptionViewerProps> = ({ 
  ...
  const checkForExistingTranscript = async (lessonId: number) => {
    try {
      ...
      // שימוש ב-API המרכזי במקום fetch ישיר
      const response = await api.get(`/Transcript/lesson/${lessonId}`);
      console.log("Transcript check response:", response.data);
      
      const transcriptInfo = response.data;
      setTranscriptName(transcriptInfo.transcriptUrl);

      if (transcriptInfo && transcriptInfo.transcriptUrl) {
        // נמצא תמלול קיים!
        console.log("Found existing transcript:", transcriptInfo);
  ...
  // הורדת תמלול מ-S3
  const fetchTranscriptFromS3 = async (transcriptUrl: string) => {
    console.log("Fetching transcript from S3:", transcriptUrl);
    
    try {
      setIsLoading(true);
      // קבלת URL חתומה להורדה באמצעות API המרכזי
      const presignedResponse = await api.get(`/upload/download-url/${transcriptUrl}`);
  ...
  const handleTranscription = async () => {
    ...
    try {
      ...
      if (selectedRecording) {
        ...
        try {
          // קבל את ה-URL ישירות מהשרת באמצעות API המרכזי
          const urlResponse = await api.get(`/upload/download-url/${selectedRecording.urlName}`);
          const downloadUrl = urlResponse.data;
      ...
      // שליחת הקובץ לתמלול באמצעות API המרכזי
      const formData = new FormData();
      formData.append("file", fileToSend);
      ...
      const response = await api.post("/transcription/transcribe-full", formData, {
        headers: {
      ...
      setIsTranscribing(false);
      const errorMessage = error.response?.data || error.message || 'שגיאה לא ידועה';
      setStatusMessage({ type: 'error', text: `שגיאה בתמלול: ${errorMessage}` });
      console.error("שגיאה בתמלול:", error);
    }
  };
  ...
  async function uploadTranscriptToS3(fileName: string, content: string): Promise<string | null> {
    try {
      // שלב 1: בקשת כתובת חתומה באמצעות API המרכזי
      const presignedResponse = await api.get(`/upload/presigned-url?fileName=${fileName}`);
      console.log("Presigned URL response:", fileName, content);
  ...
  async function saveTranscriptUrlToDb(fileName: string, name: string) {
    try {
      ...
      // שמירת המידע על התמלול שנוצר
      setExistingTranscript(response.data);
