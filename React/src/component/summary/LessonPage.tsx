...
import {Box,Typography,Grid,Card,CardContent,CardActions,Button,CircularProgress,Chip,
  Divider, Paper, Container
} from "@mui/material";
...
const LessonsPage = () => {
  ...
  const [lessonForTranscript, setLessonForTranscript] = useState<Lesson | null>(null); // הוספת state לשמירת השיעור שצריך תמלול
  ...
  const handleSummaryClick = async (lesson: Lesson) => {
    try {
      ...
      if (!transcriptUrl) {
        setLessonForTranscript(lesson); // שמירת השיעור שצריך תמלול
      ...
      setLessonForTranscript(lesson); // שמירת השיעור שצריך תמלול גם במקרה של שגיאה
      setShowNoTranscriptDialog(true);
    } finally {
      setLoadingLessons(prev => ({ ...prev, [lesson.id]: false }));
    }
  ...
  const handleKeyPointsClick = async (lesson: Lesson) => {
    try {
      setLoadingLessons(prev => ({ ...prev, [lesson.id]: true }));
  ...
  const handleCloseNoTranscriptDialog = () => {
    setShowNoTranscriptDialog(false);
    setLessonForTranscript(null); // איפוס השיעור שנשמר
  };
    ...
    <Container maxWidth="xl">
        ...
        {filesLoading ? (
          ...
          <Grid container spacing={3}>
            {allUserFiles.length === 0 ? (
              ...
              allUserFiles.map((lesson: Lesson) => (
                    ...
                    <CardContent sx={{ flexGrow: 1 }}>
                      <Box sx={{ display: 'flex', alignItems: 'center', mb: 2, backgroundColor: '#4361ee15', p: 1.5, borderRadius: 2, borderLeft: '4px solid #4361ee' }}>
                        ...
                        <Typography variant="h6" fontWeight="bold" sx={{ color: '#1a237e' }}>
                          {lesson.lessonName ? lesson.lessonName.split('.').slice(0, -1).join('.') : 'שיעור ללא שם'}
                        </Typography>
                      </Box>
                      <Divider sx={{ mb: 2 }} />
      ...
      {selectedLesson && transcriptText && activeDialog === 'summary' && (
        <ShowSummary
          ...
          lessonName={selectedLesson.lessonName || 'שיעור ללא שם'}
          onClose={handleCloseDialogs}
          onDownload={() => handleDownload(transcriptText, `סיכום-${selectedLesson.lessonName || 'שיעור'}.txt`)}
        />
      )}
      ...
      {selectedLesson && keyPointsText && activeDialog === 'keyPoints' && (
        <ShowKeyPoints
          ...
          lessonName={selectedLesson.lessonName || 'שיעור ללא שם'}
          onClose={handleCloseDialogs}
          onDownload={() => handleDownload(keyPointsText, `נקודות-חשובות-${selectedLesson.lessonName || 'שיעור'}.txt`)}
        />
      )}

      <NoTranscriptDialog
        open={showNoTranscriptDialog}
        onClose={handleCloseNoTranscriptDialog}
        selectedLesson={lessonForTranscript} // העברת השיעור הנבחר לדיאלוג
      />
    </Container>
  );
};
