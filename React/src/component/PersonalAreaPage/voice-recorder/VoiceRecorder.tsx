...
import { RecordingControlsHorizontal } from "./RecordingControlsHorizontal"
...
export const VoiceRecorder: React.FC<Id> = ({ parentId }) => {
  ...
  const [selectedFiles, setSelectedFiles] = useState<File[]>([])
  const [isDialogOpen, setIsDialogOpen] = useState<boolean>(false)
  const [_isVisible, setIsVisible] = useState<boolean>(true)
  ...
  const saveRecording = (): void => {
    if (audioBlob) {
      const fileName = lessonName.trim() ? `${lessonName.trim()}.wav` : `recording-${new Date().toISOString()}.wav`
  ...
  return (
    ...
    >
      ...
      >
        <LessonNameInput lessonName={lessonName} onChange={setLessonName} />
        ...
        {transcriptionEnabled && isTranscriptionSupported && (recording || finalTranscript) ? (
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={6}>
              <Box sx={{ display: "flex", flexDirection: "column", height: "100%" }}>
                <RecordingTimer recordingTime={recordingTime} primaryColor={primaryColor} />

                <WaveformDisplay waveformValues={waveformValues} primaryColor={primaryColor} />
