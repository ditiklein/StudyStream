    ...
    >
      ...
      >
        ...
        <IconButton
          ...
          sx={{
            ...
            left: 16,
            "&:hover": { bgcolor: "rgba(255,255,255,0.1)" },
            "&:disabled": { color: "rgba(255,255,255,0.5)" }, // 🆕 Style for disabled state
          }}
          aria-label="סגור"
        >
          <Close />
        ...
        >
          ...
          >
            ...
            <AudioControls
              ...
              onPlayPause={handlePlayPause}
              onStop={audioPlayer.handleStop}
              disabled={!audioContext.audioBuffer || isSaving} // 🆕 Disable during save
            />
          </Box>
          ...
          <Box sx={{ width: "400px", display: "flex", flexDirection: "column" }}>
            <EditControls
              ...
              endTime={audioSelection.endTime}
              duration={audioPlayer.duration}
              isLoading={audioContext.isLoading || audioProcessing.isProcessing || isSaving} // 🆕 Include save loading
              canExecuteEdit={canExecuteEdit() && !isSaving} // 🆕 Disable during save
              onEditModeChange={setEditMode}
              onStartTimeChange={audioSelection.setStartTime}
              onEndTimeChange={audioSelection.setEndTime}
              onExecuteEdit={handleExecuteEdit}
      ...
      >
        <Button
          ...
          variant="outlined"
          size="large"
          disabled={isSaving} // 🆕 Disable close during save
        >
          סגור
        </Button>
        <Button
          variant="contained"
          onClick={handleSave}
          disabled={audioContext.isLoading || !audioContext.audioBuffer || isSaving} // 🆕 Include save loading
          size="large"
          sx={{
            minWidth: 140, // 🆕 Fixed width to prevent button jumping
          }}
          startIcon={
            isSaving ? (
              <CircularProgress size={20} color="inherit" /> // 🆕 Loading spinner
            ) : (
              <Save />
            )
          }
        >
          {isSaving ? "שומר..." : "שמור שינויים"} {/* 🆕 Dynamic text */}
        </Button>
      </DialogActions>
    </Dialog>
  )
